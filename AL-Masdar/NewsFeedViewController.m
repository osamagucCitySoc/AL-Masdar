...
-(void)checkRate
{
    ...
    {
        if ([[NSUserDefaults standardUserDefaults] integerForKey:@"rateCount"] >= 5)
        {
            [self showRateView];
        }
        else
...
-(void)checkAds
{
    NSURL *storeURL = [NSURL URLWithString:@"https://dl.dropboxusercontent.com/u/59230746/Al-Masdar/almasdar-ad.txt"];
    ...
    else
    {
        ...
        }else if (diff < 604800)
        {
            ...
            if (!showingFav)
            {
                if ((int)(diff/86400) == 1)
                {
                    [(UILabel*)[tableView viewWithTag:838] setText:@"أخبار الأمس"];
                }
                else
                {
