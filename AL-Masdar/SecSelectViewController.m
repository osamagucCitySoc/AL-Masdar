...
#import "SecSelectViewController.h"

@interface SecSelectViewController ()

@end

@implementation SecSelectViewController
...
- (void)viewDidLoad
{
    ...
    if (![[NSUserDefaults standardUserDefaults] objectForKey:@"notifySubscriptions"] || [[[NSUserDefaults standardUserDefaults] objectForKey:@"notifySubscriptions"] count] == 0)
    {
        [[NSUserDefaults standardUserDefaults] setObject:[[NSUserDefaults standardUserDefaults] objectForKey:@"subscriptions"] forKey:@"notifySubscriptions"];
    }
    
    dictArray = [[NSMutableArray alloc] initWithArray:[[NSUserDefaults standardUserDefaults] objectForKey:@"subscriptions"]];
    NSMutableArray *currentSubs = [[NSMutableArray alloc] initWithArray:[[NSUserDefaults standardUserDefaults] objectForKey:@"notifySubscriptions"]];
    NSDictionary *news;
...
- (IBAction)selectAllSources:(id)sender {
    if (selectedCount < idArray.count)
    {
        ...
        [currentInstallation addUniqueObjectsFromArray:idArray forKey:@"customChannels"];
        [currentInstallation saveInBackground];
        
        [[NSUserDefaults standardUserDefaults] setObject:dictArray forKey:@"notifySubscriptions"];
    ...
    else
    {
        ...
        PFInstallation *currentInstallation = [PFInstallation currentInstallation];
        [currentInstallation removeObjectsInArray:idArray forKey:@"customChannels"];
        [currentInstallation removeObjectsInArray:soucresArr forKey:@"customChannels"];
        [currentInstallation saveInBackground];
        
        [[NSUserDefaults standardUserDefaults] setObject:[[NSMutableArray alloc] init] forKey:@"notifySubscriptions"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
...
-(void)setNewSource:(NSString*)theId andIsAdd:(BOOL)isAdd
{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:currentSubs forKey:@"notifySubscriptions"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
-(void)saveAllSources
{
    ...
    [currentInstallation setObject:[[NSArray alloc] init] forKey:@"customChannels"];
    [currentInstallation saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        if (succeeded && !error) {
            [currentInstallation addUniqueObjectsFromArray:sources forKey:@"customChannels"];
            [currentInstallation saveInBackground];
